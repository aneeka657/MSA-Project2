...
def create_dataset_original_style(
    processed_dir="/Scratch/repository/msa/MSATSUNGPING/beatles-original-preprocessed-data",
    output_file="./beatles_data/test_data.npz"
):
    """
    Create dataset following the ORIGINAL author's approach:
    - No _len.npy files (they didn't save lengths)
    - Variable-length sequences stored as object arrays
    - Length calculated from actual sequence shapes during training
    """
    ...
    for spec_file in sorted(spec_files):
        ...
        try:
            ...
            # valid_len = np.load(base_name + '_len.npy')
...
if __name__ == "__main__":
    ...
    dataset = create_dataset_original_style(
        processed_dir="./beatles-original-preprocessed-data",
        output_file="./beatles_data/test_data.npz"
    )
    
    # Verify it works
    verify_original_style_dataset("./beatles_data/test_data.npz")
