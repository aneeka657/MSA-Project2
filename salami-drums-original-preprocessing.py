...
def create_feature_label(audio_files, annotation_dict, save_dir, downsample, downsample_window, max_len=935):
    ...
    for i_f, file in enumerate(sorted(audio_files)[start:]):
        ...
        id = basename(file).replace('.wav', '').replace('.wav', '')
...
def run_salami_preprocessing_no_aug(
    ...
    audio_dir="/Scratch/repository/iahmad/salami-demucs/drums",
    annotation_dir="/Scratch/repository/iahmad/salami-mapped-annotation",
    output_dir="./salami-original-preprocessed-data",
    final_dataset_path="./salami_data/test_data.npz",
    max_len=935
):
    """
    Complete SALAMI preprocessing following author's exact method BUT WITHOUT AUGMENTATION
    """
    
    print("🎵 SALAMI Preprocessing - NO AUGMENTATION VERSION")
    print("=" * 70)
